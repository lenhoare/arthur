import { ComponentHost } from "../interactive/components";
import { getStoredComponent } from "../interactive/data";
...
class App extends React.Component<AppProps, AppState> {
    ...
    this.scene.getNonDeletedElements().filter((element) => {
      if (isEmbeddableElement(element)) {
        ...
        if (!this.embedsValidationStatus.has(element.id)) {
          ...
          const validated =
            !!getStoredComponent(element) ||
            embeddableURLValidator(element.link, this.props.validateEmbeddable);
    ...
    return (
      <>
        {embeddableElements.map((el) => {
          const component = getStoredComponent(el);
            ...
            >
              <div
                ...
                style={{
                  ...
                  overflow: component ? "visible" : undefined,
                  pointerEvents:
                    isActive && !component
              ...
              >
                ...
                <div
                  ...
                  style={{
                    padding: component ? 0 : `${el.strokeWidth}px`,
                  }}
                >
                  {(component && isEmbeddableElement(el) ? (
                    <ComponentHost
                      key={`${this.state.componentSessionId}:${el.id}`}
                      element={el}
                      app={this}
                      interactive={this.state.viewModeEnabled}
                      editing={this.state.editingComponentId === el.id}
                      finishEditing={() =>
  ...
  private handleCanvasDoubleClick = (
    ...
    const componentHit = this.getElementAtPosition(sceneX, sceneY);
    if (
      !this.state.viewModeEnabled &&
      componentHit &&
      !componentHit.locked &&
      getStoredComponent(componentHit)
    ) {
      this.setState({
        editingComponentId: componentHit.id,
        activeEmbeddable: null,
      });
      return;
    }
